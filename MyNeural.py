    ...
    def train(self, data, all_y_trues):
        ...
        for epoch in range(epochs):
            for x, y_true in zip(data, all_y_trues):
                ...
                self.w6 -= learn_rate * d_L_d_ypred * d_ypred_d_h2 * d_h1_d_w6
...
kostya = np.array([1, 1, 25])  # 128 фунтов, 63 дюйма
ivan = np.array([0, 0, 25])  # 155 фунтов, 68 дюймов
maria = np.array([1, 0, 5])
pavel = np.array([0, 1, 30])
print("Вероятность АГ Кости: %.3f" % network.feedforward(kostya))  # 0.951 - F
print("Вероятность АГ Ивана: %.3f" % network.feedforward(ivan))  # 0.039 - M
print("Вероятность АГ Марии: %.3f" % network.feedforward(maria))
print("Вероятность АГ Павла: %.3f" % network.feedforward(pavel))